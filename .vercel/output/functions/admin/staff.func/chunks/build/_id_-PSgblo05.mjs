...
//#region pages/admin/colors/[id].vue?vue&type=script&setup=true&lang.ts
var _id__vue_vue_type_script_setup_true_lang_default = /*@__PURE__*/ defineComponent({
	__name: "[id]",
	__ssrInlineRender: true,
	setup(__props) {
		useHead$1({ title: "تعديل بيانات اللون | لوحة التحكم" });
		const route = useRoute();
		const colorId = String(route.params.id);
		const { submitForm, isSubmitting } = useAdminColors();
		const loading = ref(true);
		const colorData = ref({});
		const handleUpdate = async (payload) => {
			if (await submitForm(payload, true, colorId)) navigateTo("/admin/colors");
		};
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<div><h1 class="text-2xl font-black text-slate-900">تعديل بيانات اللون (ID: ${ssrInterpolate(unref(route).params.id)})</h1><p class="text-xs sm:text-sm text-slate-500 mt-1 font-medium"> تحديث اسم اللون أو كود الـ Hex أو الصورة عبر API التعديل (POST FormData). </p></div></div>`);
			if (loading.value) _push(`<div class="flex flex-col items-center justify-center py-20 text-amber-500 gap-4"><div class="w-10 h-10 border-4 border-amber-400 border-t-transparent rounded-full animate-spin"></div><span class="font-extrabold text-sm text-slate-700">جاري جلب بيانات اللون من السيرفر...</span></div>`);
			else _push(ssrRenderComponent(ColorFormAdvanced_default, {
				"is-edit-mode": true,
				"initial-data": colorData.value,
				"is-submitting": unref(isSubmitting),
				onSubmit: handleUpdate,
				onCancel: ($event) => ("navigateTo" in _ctx ? _ctx.navigateTo : unref(navigateTo))("/admin/colors")
			}, null, _parent));
			_push(`</div>`);
		};
	}
...
//#endregion
//#region pages/admin/colors/[id].vue
...
_id__vue_vue_type_script_setup_true_lang_default.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/admin/colors/[id].vue");
	return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
var _id__default = _id__vue_vue_type_script_setup_true_lang_default;

export { _id__default as default };
//# sourceMappingURL=_id_-PSgblo05.mjs.map
