    ...
    <div class="bg-white p-4 sm:p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div>
        <h2 class="text-xl sm:text-2xl font-black text-slate-900">
          {{ isEditMode ? 'تعديل علامة تجارية' : 'إضافة علامة تجارية جديدة' }}
        </h2>
        <p class="text-xs sm:text-sm text-slate-500 font-medium mt-1">
          أدخل اسم البراند بالعربية والإنجليزية وحدد صورة الشعار (اللوجو) ليتم إرسال البيانات كـ FormData.
        </p>
      </div>
      ...
      <div class="flex items-center gap-3 w-full sm:w-auto">
        ...
        >
          ...
          <span>{{ isSubmitting ? 'جاري الإرسال (FormData)...' : (isEditMode ? 'حفظ التعديلات' : 'إضافة البراند') }}</span>
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div class="xl:col-span-2 bg-white rounded-3xl p-6 shadow-sm border border-slate-100 space-y-6">
        <h3 class="text-base font-extrabold text-slate-900 flex items-center gap-2">
          ...
          اسم العلامة التجارية (Arabic & English Brand Names)
        </h3>

        <!-- Arabic Brand Name -->
        <div class="space-y-1.5">
          <label class="text-xs font-extrabold text-[#0B0E28] block">
            اسم العلامة التجارية (بالعربية) <span class="text-rose-500">*</span>
          </label>
          <input 
            type="text" 
      ...
      <!-- Left Column: Image / Logo File Upload (1 col) -->
      <div class="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 space-y-6">
        <h3 class="text-base font-extrabold text-slate-900 flex items-center gap-2">
          <span class="w-2.5 h-2.5 rounded-full bg-amber-400"></span>
          شعار العلامة التجارية (image File Object)
        </h3>

        <div class="space-y-3 text-center">
          <!-- Current / Selected Image Preview -->
          <div class="w-32 h-32 rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden mx-auto flex items-center justify-center relative p-2">
            <img 
              v-if="imagePreview" 
              :src="imagePreview" 
            ...
            />
            <svg v-else class="w-10 h-10 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
          </div>
          ...
          <div class="p-4 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl">
            <label class="block text-xs font-bold text-slate-700 mb-2">
              اختر ملف الشعار {{ isEditMode ? '(اختياري عند التعديل)' : '(إجباري عند الإضافة)' }}
            </label>
            <input 
              type="file" 
              accept="image/*"
...
const props = defineProps<{
  initialData?: Partial<BrandFormDataPayload & { existingImage?: string }>;
  isEditMode?: boolean;
  isSubmitting?: boolean;
}>()

const emit = defineEmits<{
  (e: 'submit', payload: BrandFormDataPayload): void;
  (e: 'cancel'): void;
}>()
...
const form = reactive<BrandFormDataPayload>({
...
const handleSubmit = () => {
  ...
  if (!form.name_ar.trim()) {
    formError.value = 'يرجى كتابة اسم العلامة التجارية بالعربية.'
    return
  }

  if (!props.isEditMode && !form.imageFile) {
    formError.value = 'يرجى تحديد شعار العلامة التجارية (ملف الصورة إجباري عند الإضافة).'
    return
  }
