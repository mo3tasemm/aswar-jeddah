<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-4 mb-6">
      ...
      <div>
        <h1 class="text-2xl font-black text-slate-900">إضافة لون جديد (POST FormData)</h1>
        <p class="text-xs sm:text-sm text-slate-500 mt-1 font-medium">
          أدخل بيانات اللون وحدد نوعه (كود Hex أو صورة نقشة/خامة) لإضافته لخيارات المتجر.
        </p>
      </div>
    </div>
...
useHead({
  title: 'إضافة لون جديد | لوحة التحكم'
})

const { submitForm, isSubmitting } = useAdminColors()

const handleCreate = async (payload: ColorFormDataPayload) => {
