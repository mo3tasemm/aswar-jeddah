<template>
  <div>
    ...
    <div v-else class="bg-white p-12 rounded-3xl text-center space-y-4 border border-slate-100">
      <h3 class="text-base font-black text-slate-800">تعذر العثور على بيانات الدور الوظيفي (#{{ route.params.id }})</h3>
      <p class="text-xs text-slate-400">تأكد من صحة معرف الدور وحاول مجدداً.</p>
      <NuxtLink to="/admin/roles" class="inline-block px-4 py-2 bg-amber-400 text-[#0B0E28] font-bold text-xs rounded-xl cursor-pointer">
