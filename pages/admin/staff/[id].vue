...
const { admins, isLoadingAdmins, fetchAdmins } = useAdminRolesAndAdmins()
const currentAdmin = ref<AdminUserItem | null>(null)

onMounted(async () => {
  const adminId = route.params.id as string
  const list = await fetchAdmins(1)
  const found = list.find(a => String(a.id) === String(adminId))
  if (found) {
    currentAdmin.value = found
  }
})
</script>
