<template>
  <div class="w-full bg-slate-50/50 min-h-screen pb-16" dir="rtl">
    <div class="container mx-auto px-4 max-w-7xl">
      ...
      <!-- 2. Header & Categories -->
      <div class="mb-8">
        <h1 class="text-2xl font-bold text-[#0B0E28] mb-6 mt-6">{{ categoryName }}</h1>
      ...
      <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 w-full">
        ...
        <main class="flex-1 w-full min-w-0 space-y-6">
          ...
          <div v-else-if="products.length === 0" class="bg-white rounded-[2rem] p-12 text-center shadow-sm border border-slate-100/60 flex flex-col items-center justify-center min-h-[400px] w-full">
            ...
            <p class="text-slate-500 max-w-sm mx-auto mb-8">لم نتمكن من العثور على منتجات تطابق الفلاتر المختارة في هذا التصنيف.</p>
            <button 
              @click="resetFilters"
...
// Mock Logic: Derive a category name based on the slug for demonstration
const categoryName = computed(() => {
  const slug = categorySlug.value
  if (slug === 'cooking') return 'أجهزة الطهي'
  if (slug === 'coffee-machines') return 'ماكينات القهوة'
  return slug ? slug.replace(/-/g, ' ') : 'تصنيف المنتجات'
})

useHead({
  title: computed(() => `${categoryName.value} | أسوار جدة`)
})

// === Breadcrumbs ===
const breadcrumbTiers = computed(() => [
  { name: 'الأجهزة الكهربائية', path: '/shop' },
  { name: categoryName.value, path: `/category/${categorySlug.value}` }
])

// === Categories Bar (Mock Data) ===
// Here we mock subcategories. In a real app, you'd fetch this based on the current slug.
...
// Refetch if the slug changes
watch(() => route.params.slug, () => {
  currentPage.value = 1
  // You would typically refetch subcategories here too based on the new slug
  fetchProducts()
})
