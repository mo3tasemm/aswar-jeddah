...
export interface PrivacyPolicyData {
  ...
  // Top 4 Trust & Security Badges
  badge_1_icon?: string
  badge_1_title_ar?: string
  badge_1_title_en?: string
  badge_1_desc_ar?: string
  badge_1_desc_en?: string

  badge_2_icon?: string
  badge_2_title_ar?: string
  badge_2_title_en?: string
  badge_2_desc_ar?: string
  badge_2_desc_en?: string

  badge_3_icon?: string
  badge_3_title_ar?: string
  badge_3_title_en?: string
  badge_3_desc_ar?: string
  badge_3_desc_en?: string

  badge_4_icon?: string
  badge_4_title_ar?: string
  badge_4_title_en?: string
  badge_4_desc_ar?: string
  badge_4_desc_en?: string

  // Quick Summary Card
  summary_title_ar?: string
  summary_title_en?: string
  summary_point_1_ar?: string
  summary_point_1_en?: string
  summary_point_2_ar?: string
  summary_point_2_en?: string
  summary_point_3_ar?: string
  summary_point_3_en?: string

  // Privacy Inquiries CTA Box
  inquiry_title_ar?: string
  inquiry_title_en?: string
  inquiry_desc_ar?: string
  inquiry_desc_en?: string
  inquiry_contact_url?: string
  inquiry_contact_btn_ar?: string
  inquiry_contact_btn_en?: string
  inquiry_whatsapp_url?: string
  inquiry_whatsapp_btn_ar?: string
  inquiry_whatsapp_btn_en?: string
...
export const privacyPolicyApiService = {
  /**
   * 1. GET Public Privacy Policy Data
   */
  async fetchPrivacyPolicy(lang = 'ar', force = false): Promise<{ success: boolean; data: PrivacyPolicyData }> {
   ...
   */
  async fetchAdminPrivacyPolicy(token: string, force = false): Promise<{ success: boolean; data: PrivacyPolicyData }> {
    const apiBase = getApiBaseUrl()
    const endpoints = [
      `${apiBase}/admin/privacy-policy`,
      `${apiBase}/admin/privacy_policy`,
   ...
   */
  async updateAdminPrivacyPolicy(token: string, payload: FormData | Record<string, any>): Promise<{ success: boolean; message?: string; data?: PrivacyPolicyData }> {
    const apiBase = getApiBaseUrl()
    const endpoints = [
      { url: `${apiBase}/admin/privacy-policy`, method: 'POST' },
      { url: `${apiBase}/admin/privacy-policy`, method: 'PUT' },
      { url: `${apiBase}/admin/privacy_policy`, method: 'POST' },
      { url: `${apiBase}/admin/privacy_policy`, method: 'PUT' }
    ]
   ...
   */
  normalizePrivacyPolicy(raw: any, lang = 'ar'): PrivacyPolicyData {
    if (!raw || typeof raw !== 'object') {
      return this.getDefaultPrivacyPolicy(lang)
    }
    ...
    if (item.privacy_policy && typeof item.privacy_policy === 'object') {
      item = item.privacy_policy
    }
   ...
   */
  getDefaultPrivacyPolicy(lang = 'ar'): PrivacyPolicyData {
    const isEn = lang === 'en'
    return {
      id: 1,
      ...
      subtitle: isEn 
        ? 'Your privacy and data security are our highest priority. Learn how we safeguard your personal information.' 
        : 'خصوصيتكم وأمان بياناتكم هي أولويتنا القصوى. تعرف على كيفية حماية وإدارة بياناتكم وفق الأنظمة المعتمدة في المملكة.',
